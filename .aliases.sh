...
RUN_CONTAINER='docker run --rm -it -v `pwd`:/qulacs-osaka'
RUN_CONTAINER_UNPRIVILEGED='docker run --rm -it -v `pwd`:/qulacs-osaka -e LOCAL_UID=`id -u $USER` -e LOCAL_GID=`id -g $USER`'
