CONTAINER_NAME=ghcr.io/qulacs-osaka/qulacs-ubuntu-pyenv:latest
CONTAINER_NAME_FORMAT=ghcr.io/qulacs-osaka/qulacs-ubuntu-clang-format:latest

# Use single quote to avoid variable expansion.
RUN_CONTAINER='docker run --rm -it -v `pwd`:/qulacs-osaka -e USE_TEST=Yes'
RUN_CONTAINER_UNPRIVILEGED='docker run --rm -it -v `pwd`:/qulacs-osaka -e LOCAL_UID=`id -u $USER` -e LOCAL_GID=`id -g $USER` -e USE_TEST=Yes'
